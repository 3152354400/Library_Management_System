...
namespace backend.Controllers
{
    ...
    public class BorrowingController : ControllerBase
    {
        ...
        /**
         * 通过读者ID获取该读者的所有借阅记录（分页）
         * @param pageSize 每页数量，默认10
         * @param pageNum 页码，默认1
         * @param status 状态筛选（可选）：未归还/已归还/全部
         * @return 分页后的借阅记录
         */
        [HttpGet("reader/paged")]
        public async Task<ActionResult<PagedResult<BorrowRecordDetailDto>>> GetBorrowRecordsByReaderPaged(
            ...
            [FromQuery] string status = null)
        {
            ...
            var result = await _borrowingService.GetBorrowRecordsByReaderPagedAsync(
                reader.ReaderID.ToString(), pageSize, pageNum, status);
            return Ok(result);
        }
