...
namespace backend.Services.Book
{
    ...
    public class BookCategoryService
    {
        ...
        // 缓存配置
        private const string LeafCategoriesCacheKey = "book_category:leaf_categories";
        private static readonly TimeSpan CacheExpiry = TimeSpan.FromMinutes(30); // 缓存30分钟
        ...
        public async Task<IEnumerable<CategorySelectDto>> GetLeafCategoriesAsync()
        {
            ...
            try
            {
                var cached = await _redis.GetCacheAsync<List<CategorySelectDto>>(LeafCategoriesCacheKey);
                if (cached != null)
                {
                    return cached;
                }
            ...
            // 从数据库获取
            var result = await _bookCategoryRepository.GetLeafCategoriesAsync();
            var resultList = result.ToList();

            // 存入缓存（缓存失败不影响返回结果）
            try
            {
                await _redis.SetCacheAsync(LeafCategoriesCacheKey, resultList, CacheExpiry);
            }
            catch (Exception ex)
            {
        ...
        {
            try { await _redis.DeleteAsync(LeafCategoriesCacheKey); }
