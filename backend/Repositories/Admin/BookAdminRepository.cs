    ...
    public class BookAdminRepository
    {
        ...
        public async Task CreateBookAsync(CreateBookDto dto)
        {
            ...
            try
            {
                ...
                var bookInfoSql = "INSERT INTO BookInfo (ISBN, Title, Author, TotalStock, AvailableStock) VALUES (:ISBN, :Title, :Author, :Stock, :Stock)";
                ...
                {
                    var bookSql = "INSERT INTO Book (Status, Barcode, ISBN) VALUES ('正常', :Barcode, :ISBN)";
                    // Dapper can execute the same command multiple times with a list of parameters
                    var booksToInsert = new List<object>();
                    var ts = DateTime.Now.ToString("HHmmssfff");
                    for (int i = 0; i < dto.NumberOfCopies; i++)
                    {
                        booksToInsert.Add(new { ISBN = dto.ISBN, Barcode = "BK" + ts + i.ToString() });
                    }
        ...
        public async Task AddCopiesAsync(AddCopiesDto dto)
        {
            ...
            try
            {
                ...
                if (dto.NumberOfCopies > 0)
                {
                    var bookSql = "INSERT INTO Book (Status, Barcode, ShelfID, ISBN) VALUES ('正常', :Barcode, :ShelfID, :ISBN)";
                    var booksToInsert = new List<object>();
                    var ts = DateTime.Now.ToString("HHmmssfff");
                    for (int i = 0; i < dto.NumberOfCopies; i++)
                    {
                        booksToInsert.Add(new { ShelfID = dto.ShelfID, ISBN = dto.ISBN, Barcode = "BK" + ts + i.ToString() });
                    }
                    await connection.ExecuteAsync(bookSql, booksToInsert, transaction);
                }

                // 步骤 2: 更新 BookInfo 表中的总库存量 (TotalStock / AvailableStock)
                var bookInfoSql = "UPDATE BookInfo SET TotalStock = TotalStock + :NumberOfCopies, AvailableStock = AvailableStock + :NumberOfCopies WHERE ISBN = :ISBN";
