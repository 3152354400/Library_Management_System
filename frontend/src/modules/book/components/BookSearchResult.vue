        ...
        <div v-for="book in filteredBooks" :key="book.ISBN" class="wr_suggestion_card_wrapper relative" >
          <div class="wr_suggestion_card_content pb-16">
            ...
            <div class="mt-4 text-center ">
              <router-link :to="{ path: '/book/detail', query: { isbn: book.ISBN } }" class="text-base font-semibold leading-tight text-blue-600 hover:underline">
                {{ book.Title }}
              </router-link>
...
// 打开预约弹窗
function openReserve(book) {
  reserveTarget.value = book
}

// 预约成功回调
function onReserveSuccess(data) {
  // 可以刷新页面或做其他处理
}
// 初次加载
...
.comments-button, .physical-books-button, .reserve-button {
  width: 120px;
}
