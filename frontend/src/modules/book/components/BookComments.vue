...
async function submitComment() {
  ...
  if (!userStore.user?.readerID && !userStore.user?.ReaderID) {
    error.value = '请先登录后再发表评论'
    return
  }

  const commentData = {
    ReaderID: userStore.user.readerID || userStore.user.ReaderID || userStore.user.ReaderID,
...
async function submitReport(comment) {
  ...
  if (!userStore.user?.readerID && !userStore.user?.ReaderID) {
    error.value = '请先登录后再举报'
    return
  }

  // 构造举报数据（后端会自动填充ReaderID）
  const reportData = {
